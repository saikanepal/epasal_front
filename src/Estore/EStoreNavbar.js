...
const EStoreNavbar = ({ toggleColorPicker }) => {
    ...
    // Destructuring positions from navbar object
    const { logoPosition, storeNamePosition, categoriesPosition, searchBarPosition } = navbar;
    ...
    return (
        ...
        >
            ...
            <Draggable
                ...
                onStop={(e, data) => setStore(prevState => ({
                    ...prevState,
                    navbar: {
                        ...prevState.navbar,
            ...
            <Draggable
                defaultPosition={storeNamePosition}
                onStop={(e, data) => setStore(prevState => ({
                    ...prevState,
                    navbar: {
                        ...prevState.navbar,
                        storeNamePosition: { x: data.x, y: data.y }
                    }
                }))}
            >
                <motion.div className="flex items-center">
                    {previewMode &&
                        <span className="font-Orbitron font-bold text-lg" style={{ color: color.navColor.storeNameTextColor, backgroundColor: color.navColor.backgroundnavColor }}>
                            {store.name}
                        </span>
                    }
                    {!previewMode && <input
                        type="text"
                        value={store.name}
                        onChange={(e) => setStore(prevState => ({ ...prevState, name: e.target.value }))}
                        className="hidden md:block font-Orbitron w-60 font-bold text-lg outline-none border-none ml-4"
            ...
            <Draggable
                defaultPosition={categoriesPosition}
                onStop={(e, data) => setStore(prevState => ({
                    ...prevState,
                    navbar: {
                        ...prevState.navbar,
                        categoriesPosition: { x: data.x, y: data.y }
                    }
                }))}
            >
                <motion.div>
                    <div className="hidden md:flex flex-grow justify-center ml-16 font-semibold font-Genos text-2xl space-x-2">
                        <ul className="flex">
            ...
            {/* Draggable Searchbar */}
            <Draggable
                defaultPosition={searchBarPosition}
                onStop={(e, data) => setStore(prevState => ({
                    ...prevState,
                    navbar: {
                        ...prevState.navbar,
                        searchBarPosition: { x: data.x, y: data.y }
                    }
                }))}
            >
                <motion.div>
