...
const SharedNavbar = ({ storeLogo, storeName, open, setOpen }) => {
  ...
  // Prevent propagation of click events to document
  const handleClick = (e) => {
    ...
    <div className="w-full font-Roboto t-0 bg-white shadow-md z-10 px-6 flex items-center justify-between h-[88px] text-black">
      <div className="flex flex-row">
        ...
        <div
          ...
          className="cursor-pointer font-bold text-xl bg-gra flex items-center"
        >
          {storeName}
        </div>
      </div>
      <div
        className={`flex items-center justify-center ml-2 mr-0 sm:ml-12 p-1 cursor-pointer rounded-lg sm:p-1 md:p-2`}
        onClick={handleClick} // Use the updated handleClick function
      >
        <HiMenuAlt3 className="mr-2" size={26} />
