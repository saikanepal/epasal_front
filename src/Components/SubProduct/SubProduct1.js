...
const SubProduct1 = ({
  ...
  const selectedSubCategory =
    store.selectedSubCategory || subCategories[0].name;
  ...
  const handleRemoveProduct = (productId) => {
    setStore((prevStore) => ({
      ...prevStore,
      products: prevStore.products.filter(
  ...
  return (
    <div className="">
      <CategorySelector />
      <div
        className="px-8 pb-8 font-Cinzel overflow-x-scroll"
        style={{
          maxWidth: "100vw",
          backgroundColor: subProductColor.categoryColor,
        }}
        {...events}
        ref={ref}
      >
        <div
          ref={containerRef}
          className="flex py-4 gap-5 pb-6 overflow-x-scroll scrollbar-thumb-gray-400 scrollbar-track-gray-100 scrollbar"
        >
          <style>{`
          ...
          {!previewMode && (
            <div className="flex-none mr-4">
              <button
                ...
                className="flex flex-col items-center justify-center w-32 h-40 border border-dashed border-gray-300 rounded-md hover:bg-gray-50/50 focus:outline-none"
              >
                <svg
