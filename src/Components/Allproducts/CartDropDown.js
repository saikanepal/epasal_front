...
const CartDropdown = ({ cart, addToCart, deleteFromCart, setStore, backgroundColor, store }) => {
    ...
    useEffect(() => {
        ...
        if (!storeData || JSON.parse(storeData).name !== store?.name) {
    ...
    useEffect(() => {
        ...
        if (validCartItems.length !== cart.length) {
            setStore(prevState => ({
                ...prevState,
                cart: validCartItems,
                cartCount: validCartItems.reduce((count, item) => count + item.count, 0)
            }));
        }
    }, [cart, store.products, setStore]);
    ...
    const handleIncreaseQuantity = (index) => {
        ...
        setStore(prevState => ({
            ...prevState,
            cart: updatedCart,
            cartCount: updatedCart.reduce((count, item) => count + item.count, 0)
        }));
    };
