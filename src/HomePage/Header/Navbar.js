...
const Navbar = ({ navbarImage, setStores }) => {
    ...
    const [showSvgLogo, setShowSvgLogo] = useState(true);
    ...
    return (
        <motion.div 
        className={`py-3 ml-3.5 mr-0.5 mt-2 fixed z-50 top-0 left-0 w-[98%] transition duration-500 rounded-xl ${visible ? 'translate-y-0' : '-translate-y-full'} ${isScrolled ? 'bg-orange-200 bg-opacity-100' : 'bg-transparent'}`}
                                        ...
                                        <div key={i} className='flex gap-4 py-1 items-center'>
                                            <Link to={`/store/${n.name}`} target='_blank'>
                                                <div className='font-bold flex flex-row gap-4 items-center'>
                                                    <img className='h-8 w-8 rounded-full border border-2 border-gray-700' src={n.logo.logoUrl} alt={n.name} />
                                                    <div>{n.name}</div>
                                                </div>
                                            </Link>
                                        </div>
                ...
                <div className='flex gap-3 items-center md:mr-0'>
                    <div className='flex items-center gap-1 text-white hidden md:flex'>
                        <div className='flex gap-16'>
                            ...
                            <div>
                                {!auth.token && (
                                    <div onClick={() => { navigate('/login') }} className='font-bold flex flex-row px-5 h-10 rounded-lg bg-[#F38825] text-white hidden cursor-pointer items-center md:flex justify-center'>
                                        Login
                                        <IoIosLogIn className='text-xl font-bold ml-2 items-center' />
                                    </div>
                                )}
                                {auth.token && (
                                    <div onClick={() => { auth.logout(); }} className='font-bold flex flex-row px-5 h-10 rounded-lg bg-[#F38825] text-white hidden items-center cursor-pointer md:flex justify-center'>
                                        Logout
                    ...
                    <div className='relative'>
                        <motion.div
                            ...
                            transition={{ duration: 0.5 }}
                            className='h-10 w-10 rounded-lg mr-6 bg-[#F38825] text-white flex flex-col items-center md:hidden justify-center'
                        >
                            <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <line x1="1" y1="1" x2="15" y2="1" stroke="white" strokeWidth="2" strokeLinecap="round" />
                                <line x1="1" y1="7" x2="15" y2="7" stroke="white" strokeWidth="2" strokeLinecap="round" />
