        ...
        <div className={`py-3 absolute z-30 top-0 left-0 flex justify-between w-[98%] relative ${scrolledFromTop ? '' : 'bg-transparent'} transition duration-500 rounded-xl grid grid-cols-2 lg:grid-cols-3`} >
            ...
            <div className='w-full hidden lg:block '>
                <ul className='flex justify-between items-center h-full'>
                    <li className='border-b-4 border-black px-3 cursor-pointer'>Home</li>
                    <li className='px-3 cursor-pointer' onClick={e => handleClickNavigation(e, footerRef)}>Contact Us</li>
                    <li className='px-3 cursor-pointer' onClick={e => handleClickNavigation(e, subscriptionPlanRef)}>Subscription</li>
                    <li className='px-3 cursor-pointer' onClick={e => handleClickNavigation(e, storeRef)}>Stores</li>
                </ul>
            </div>
            <div className='flex justify-end gap-2 md:gap-5 mr-0 md:mr-10 items-center'>
                <Link to="/settings"><span className='flex gap-2 bg-[#112D4E] h-[40px] w-[120px] rounded-full  items-center justify-center text-white'>Settings <IoMdSettings /> </span></Link>


                <Link to='/buildStore' target='_blank' className='hidden md:flex bg-[#112D4E] h-[40px] w-[104px] rounded-full items-center justify-center text-white pl-4 gap-2 text-sm md:text-base'>Build <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-pencil-plus"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M4 20h4l10.5 -10.5a2.828 2.828 0 1 0 -4 -4l-10.5 10.5v4" /><path d="M13.5 6.5l4 4" /><path d="M16 19h6" /><path d="M19 16v6" /></svg></Link>


                {!auth.token && <div onClick={() => { navigate('/login') }} className='bg-[#112D4E] h-[40px] w-[104px] rounded-full cursor-pointer flex justify-center items-center text-white pl-4 gap-2 text-sm md:text-base'>
                    Login
                    <IoIosLogIn />
                </div>}
                {auth.token && <div onClick={() => { auth.logout(); }} className='bg-[#112D4E] h-[40px] w-[104px] rounded-full cursor-pointer flex items-center text-white  pl-4 gap-2'>
                    Logout
                    <IoLogOut />
                </div>}
            </div>
